...
func (cc *CampaignController) Update(c *gin.Context) {
	...
	campaign.MainImage = req.MainImage
...
// UploadMainImage 上传计划主图
func (cc *CampaignController) UploadMainImage(c *gin.Context) {
	idStr := c.Param("id")
	...
	if err != nil {
		utils.BadRequest(c, "请选择要上传的主图文件")
		return
	}

	// 保存文件
	uploadResp, err := utils.SaveUploadedFile(c, file, "campaigns/images")
	if err != nil {
	...
	if campaign.MainImage != "" {
		utils.DeleteFile(campaign.MainImage)
	}

	// 更新计划主图字段
	campaign.MainImage = uploadResp.URL
	if err := cc.campaignService.Update(campaign); err != nil {
		utils.InternalServerError(c, "更新计划主图失败")
		return
	}
...
// UploadVideo 上传计划视频
func (cc *CampaignController) UploadVideo(c *gin.Context) {
	idStr := c.Param("id")
	id, err := strconv.ParseUint(idStr, 10, 32)
	if err != nil {
		utils.BadRequest(c, "无效的计划ID")
	...
	if err != nil {
		utils.BadRequest(c, "请选择要上传的视频文件")
		return
	}

	// 保存文件
	uploadResp, err := utils.SaveUploadedFile(c, file, "campaigns/videos")
	if err != nil {
		utils.BadRequest(c, "上传失败: "+err.Error())
		return
	}
	...
	// 删除旧视频文件
	if campaign.Video != "" {
		utils.DeleteFile(campaign.Video)
	}

	// 更新计划视频字段
	campaign.Video = uploadResp.URL
	if err := cc.campaignService.Update(campaign); err != nil {
		utils.InternalServerError(c, "更新计划视频失败")
		return
	}
