...
func setupProtectedRoutes(r *gin.RouterGroup) {
	...
	{
		dashboard.GET("/stats", dashboardController.GetStats)  // 仪表盘统计数据
		dashboard.GET("/activities", dashboardController.GetActivities)  // 最近活动
		dashboard.GET("/revenue-trend", dashboardController.GetRevenueTrend)  // 收入趋势
		dashboard.GET("/user-growth", dashboardController.GetUserGrowth)  // 用户增长
		dashboard.GET("/campaign-performance", dashboardController.GetCampaignPerformance)  // 计划表现
		dashboard.GET("/realtime", dashboardController.GetRealtime)  // 实时数据
	}
...
func setupAdminRoutes(r *gin.RouterGroup) {
	...
	roles := r.Group("/roles")
	{
		roles.GET("", permissionController.GetAllRoles)
		roles.POST("", permissionController.CreateRole)
		roles.PUT("/:id", permissionController.UpdateRole)
	...
	campaigns := r.Group("/campaigns")
	{
		...
		campaigns.POST("/:id/upload-logo", campaignController.UploadLogo)
