...
func (s *PermissionService) buildMenuTree(permissions []models.Permission) []models.Permission {
	...
	// 构建树形结构 - 使用指针数组
	var treePointers []*models.Permission
	for i := range permissions {
		if permissions[i].ParentID == 0 {
			treePointers = append(treePointers, &permissions[i])
		} else {
	...
	// 转换为值数组返回
	tree := make([]models.Permission, len(treePointers))
	for i, p := range treePointers {
		tree[i] = *p
	}
